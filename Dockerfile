...
RUN cd /dist/src/app && export SSH_KEY="/dist/src/app/id_rsa" \
    && chmod +x /dist/src/app/install_ssh_eky.sh && /dist/src/app/install_ssh_eky.sh \
    && npm ci --force && npm run cp-libs
